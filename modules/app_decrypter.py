...
def run(self=None):
    while True:
        ...
        if choice == '0':
                    ...
                    if sec['name'] == sec_name:
                        ...
                        if method and dec_data:
                            ...
                            with open(out_path, 'wb') as f:
                                f.write(bytes(patched))
                            cprint(f"\n  \033[92mSaved to: {out_path}\033[0m")
                        break
                else:
                    cprint(f"  \033[91mSection '{sec_name}' not found\033[0m")
            pause()
        elif choice == '5':
            folder = prompt("\033[33m  folder path > \033[0m").strip().strip('"').strip("'")
            if not os.path.isdir(folder):
                cprint("  \033[91m[X] Folder not found\033[0m")
                pause()
                continue
            ...
            if not files:
                cprint("  \033[91m[X] No executable files found in folder\033[0m")
                pause()
                continue
            out_dir = os.path.join(folder, "decrypted_output")
            os.makedirs(out_dir, exist_ok=True)
            clear()
            cprint(f"\n  \033[36mBatch decrypt: {len(files)} files\033[0m\n")
            log_all = []
            for i, fp in enumerate(files, 1):
                cprint(f"  \033[97m[{i}/{len(files)}] {os.path.basename(fp)}...\033[0m")
                ...
                log_all.extend(log)
                col = "\033[92m" if patched_count else "\033[93m"
                cprint(f"    \033[0m-> {col}{patched_count} sections decrypted\033[0m")
            log_path = os.path.join(out_dir, "batch_log.txt")
            with open(log_path, 'w', encoding='utf-8', errors='replace') as f:
                f.write('\n'.join(log_all))
            cprint(f"\n  \033[92mBatch complete! Output in: {out_dir}\033[0m")
            pause()
        elif choice == '6':
            ...
            if os.name == 'nt':
                try:
                    ...
                    for line in result.stdout.strip().split('\n'):
                        ...
                        if len(parts) >= 5:
                            found.append({"name": parts[0], "pid": parts[1], "mem": parts[4]})
            ...
            if found:
                ...
                for i, p in enumerate(found, 1):
                    if os.name == 'nt':
                        cprint(f"    \033[97m[{i}] PID: {p['pid']:<8} Name: {p['name']:<25} Memory: {p['mem']}\033[0m")
                    else:
                        cprint(f"    \033[97m[{i}] PID: {p['pid']:<8}  {p.get('cmd', '')[:60]}\033[0m")
                sel = prompt("\033[33m  auto-decrypt first process? (y/n) > \033[0m")
                if sel.lower() == 'y' and os.name == 'nt':
                    pid = int(found[0]['pid'])
